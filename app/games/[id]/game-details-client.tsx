...
export default function GameDetailsClient({ gameId }: { gameId: string }) {
  ...
  useEffect(() => {
    if (!profileCheckLoading && !authLoading && user && !isProfileComplete) {
      sessionStorage.setItem("pendingBookingGameId", gameId)
    }
  }, [profileCheckLoading, authLoading, user, isProfileComplete, gameId])
  ...
  const handleBookGame = async () => {
    ...
    try {
      const result = await bookGameSlot(gameId, {
      ...
      <div className="container mx-auto max-w-4xl">
        <Card className="border-border bg-card text-card-foreground shadow-xl">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground">Book your spot for this game</CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-4">
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {tags.map((tag: string) => (
                      <span key={tag} className="px-2 py-1 bg-duck-orange/20 text-duck-orange text-xs">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              <div className="bg-muted p-4">
                ...
                {bookings.length > 0 ? (
                  ...
                  <p className="text-muted-foreground">No players have booked this game yet. Be the first!</p>
                )}
          ...
          <CardFooter className="flex flex-col space-y-4">
            {game.status === "open" && spotsAvailable > 0 ? (
              <>
                {hasBooked ? (
                  <div className="w-full p-3 bg-green-900/20 border border-green-900 text-green-400 text-center">
              ...
              <div className="w-full p-3 bg-red-900/20 border border-red-900 text-red-400 text-center">
